...
width=0.2
_w=width/2
plt.bar([i-(width/2) for i in ori],count_ori,            width,color="black",label=u'A:载体图像')
plt.bar([i+(width/2) for i in embeded],count_embeded,    width,color="b",label=u'B:载密图像')
plt.bar([i-1.5*width for i in aised],count_ais,          width,color="r",label=u'C:处理后载体图像')
plt.bar([i+1.5*width for i in embed_ais],count_embed_ais,width,color="g",label=u'D:处理后载密图像')

#plt.xticks(np.arange(-10,10,1))

plt.text(-(width/2),count_ori[index_ori], 'A',           ha='center', va='bottom', fontsize=10)
plt.text((width/2),count_embeded[index_embeded], 'B',    ha='center', va='bottom', fontsize=10)
plt.text(-1.5*width,count_ais[index_aised], 'C',         ha='center', va='bottom', fontsize=10)
plt.text(1.5*width,count_embed_ais[index_embed_ais], 'D',ha='center', va='bottom', fontsize=10)
